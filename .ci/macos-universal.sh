...
tar xvf $BASE_ARTIFACT/*.tar.gz -C $BASE_ARTIFACT
mv $(find $BASE_ARTIFACT -type d -mindepth 1 -maxdepth 1)/* $BUNDLE_DIR
...
for OTHER_ARTIFACT in "${ARTIFACTS_LIST[@]:1}"; do
    ...
    tar xvf $OTHER_ARTIFACT/*.tar.gz -C $OTHER_ARTIFACT
    mv $(find $OTHER_ARTIFACT -type d -mindepth 1 -maxdepth 1)/* $OTHER_ARTIFACT
