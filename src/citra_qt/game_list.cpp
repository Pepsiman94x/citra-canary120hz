...
void GameList::UninstallTitles(
    ...
    (void)QtConcurrent::run([this, titles] {
        for (auto& title : titles) {
            ...
            emit main_window->UninstallReport(UninstallStatus::Started, name);
            ...
            if (result.IsError()) {
                LOG_ERROR(Frontend, "Failed to uninstall '{}': 0x{:08X}", name.toStdString(),
                          result.raw);
                emit main_window->UninstallReport(UninstallStatus::Failed, name);
            } else {
                emit main_window->UninstallReport(UninstallStatus::Succeeded, name);
            }
        }
    });
}
