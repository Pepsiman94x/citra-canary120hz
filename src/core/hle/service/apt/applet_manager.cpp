...
static void CaptureFrameBuffer(Core::System& system, u32 capture_offset, VAddr src, u32 height,
    ...
    if (format == UINT32_MAX) {
        // This format is used to indicate the framebuffer should not be copied.
        return;
    }

    // Address in VRAM that APT copies framebuffer captures to.
    static constexpr auto screen_capture_base_vaddr = Memory::VRAM_VADDR + 0x500000;
    ...
    for (u32 y = 0; y < height; y++) {
        for (u32 x = 0; x < GSP::FRAMEBUFFER_WIDTH; x++) {
            auto dst_offset = VideoCore::GetMortonOffset(x, y, bpp) +
                              (y & ~7) * GSP::FRAMEBUFFER_WIDTH_POW2 * bpp;
            auto src_offset = bpp * (GSP::FRAMEBUFFER_WIDTH * y + x);
...
void AppletManager::ButtonUpdateEvent(std::uintptr_t user_data, s64 cycles_late) {
    ...
    Core::System::GetInstance().CoreTiming().ScheduleEvent(
        usToCycles(button_update_interval_us) - cycles_late, button_update_event);
}
...
AppletManager::AppletManager(Core::System& system) : system(system) {
    ...
    HLE::Applets::Init();
    button_update_event = Core::System::GetInstance().CoreTiming().RegisterEvent(
        "APT Button Update Event", [this](std::uintptr_t user_data, s64 cycles_late) {
            ButtonUpdateEvent(user_data, cycles_late);
        });
    Core::System::GetInstance().CoreTiming().ScheduleEvent(usToCycles(button_update_interval_us),
                                                           button_update_event);
}
