...
void Module::Interface::GetTaskIdList(Kernel::HLERequestContext& ctx) {
    ...
    if (auto* task_list_prop =
            std::get_if<std::vector<u8>>(&cur_props.props[PropertyID::TASKIDLIST]);
        task_list_prop && task_list_prop->size() == taskidlist_size) {
...
std::vector<FileSys::Entry> Module::Interface::GetBossExtDataFiles() {
...
void Module::Interface::StartTask(Kernel::HLERequestContext& ctx) {
    ...
    if (size > 0x8) {
        ...
        if (!task_id_list.contains(task_id)) {
            ...
            if (const auto* url_prop = task_id_list[task_id].props.contains(PropertyID::URL)
                ...
                std::string_view file_name(task_id_list.find(task_id)->first.c_str(),
