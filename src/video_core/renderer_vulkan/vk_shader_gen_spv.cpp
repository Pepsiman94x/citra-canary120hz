...
void FragmentModule::DefineTexSampler(u32 texture_unit) {
    ...
    if (texture_border_color.enable_s || texture_border_color.enable_t) {
        const Id texcoord_pointer_id{TypePointer(spv::StorageClass::Function, f32_id)};
        const Id texcoord_s{
            OpLoad(f32_id, OpAccessChain(texcoord_pointer_id, texcoord, ConstU32(0u)))};
        const Id texcoord_t{
            OpLoad(f32_id, OpAccessChain(texcoord_pointer_id, texcoord, ConstU32(1u)))};
