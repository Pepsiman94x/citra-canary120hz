...
void TextureRuntime::Reset() {
    ...
    for (const auto& [tag, alloc] : alloc_cache) {
        if (alloc.images[0].handle) {
            vmaDestroyImage(allocator, alloc.images[0].handle, alloc.images[0].allocation);
        }
        if (alloc.images[1].handle) {
            vmaDestroyImage(allocator, alloc.images[1].handle, alloc.images[1].allocation);
        }
        if (alloc.images[2].handle) {
            vmaDestroyImage(allocator, alloc.images[2].handle, alloc.images[2].allocation);
        }
    }
    alloc_cache.clear();
}
...
void TextureRuntime::Recycle(Allocation&& alloc) {
...
Surface::~Surface() {
    ...
    runtime->Recycle(std::move(alloc));
}
