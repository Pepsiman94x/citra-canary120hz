...
void Swapchain::SetPresentMode() {
    ...
    // If vsync is enabled and the frame limit does not have the default value
    // assume the user intends to speedup/slowdown the game and attempt to use
    // mailbox mode to not block the frame limiter. If mailbox is not available
    // use immediate and warn about it.
