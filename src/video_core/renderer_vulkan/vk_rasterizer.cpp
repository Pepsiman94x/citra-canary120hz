...
RasterizerVulkan::RasterizerVulkan(Memory::MemorySystem& memory,
    ...
    // When command buffers are submitted all dynamic state is lost.
    // Add a callback so the scheduler can reset it after flushing.
    scheduler.SubmitCallback([this] { SyncFixedState(); });
}
