...
bool RasterizerVulkan::Draw(bool accelerate, bool is_indexed) {
    ...
    const bool write_depth_fb =
        ...
        (has_stencil && regs.framebuffer.output_merger.stencil_test.enable &&
         regs.framebuffer.output_merger.stencil_test.write_mask != 0);
...
void RasterizerVulkan::SyncCullMode() {
    ...
    scheduler.Record([cull_mode = regs.rasterizer.cull_mode](vk::CommandBuffer cmdbuf) {
        cmdbuf.setCullModeEXT(PicaToVK::CullMode(cull_mode));
        cmdbuf.setFrontFaceEXT(PicaToVK::FrontFace(cull_mode));
    });
}

void RasterizerVulkan::SyncTopology(bool accelerate) {
    const auto topology = accelerate ? regs.pipeline.triangle_topology.Value()
                                     : Pica::PipelineRegs::TriangleTopology::List;

    if (!instance.IsExtendedDynamicStateSupported()) {
        pipeline_info.rasterization.topology.Assign(topology);
        return;
    }

    scheduler.Record([topology](vk::CommandBuffer cmdbuf) {
        cmdbuf.setPrimitiveTopologyEXT(PicaToVK::PrimitiveTopology(topology));
    });
}
...
void RasterizerVulkan::SyncBlendColor() {
    const u32 blend_color = regs.framebuffer.output_merger.blend_const.raw;
    scheduler.Record([blend_color](vk::CommandBuffer cmdbuf) {
        const Common::Vec4f color = PicaToVK::ColorRGBA8(blend_color);
...
void RasterizerVulkan::SyncStencilWriteMask() {
    const u8 write_mask = (regs.framebuffer.framebuffer.allow_depth_stencil_write != 0)
                              ? regs.framebuffer.output_merger.stencil_test.write_mask.Value()
                              : 0;

    scheduler.Record([write_mask](vk::CommandBuffer cmdbuf) {
        cmdbuf.setStencilWriteMask(vk::StencilFaceFlagBits::eFrontAndBack, write_mask);
    });
}
...
void RasterizerVulkan::SyncDepthWriteMask() {
    ...
    scheduler.Record(
        [write_enable](vk::CommandBuffer cmdbuf) { cmdbuf.setDepthWriteEnableEXT(write_enable); });
}
...
void RasterizerVulkan::SyncStencilTest() {
    ...
    scheduler.Record([this, stencil_test, test_enable](vk::CommandBuffer cmdbuf) {
        ...
        cmdbuf.setStencilTestEnableEXT(test_enable);
...
void RasterizerVulkan::SyncDepthTest() {
    ...
    scheduler.Record([test_enabled, compare_op](vk::CommandBuffer cmdbuf) {
        cmdbuf.setDepthTestEnableEXT(test_enabled);
        cmdbuf.setDepthCompareOpEXT(PicaToVK::CompareFunc(compare_op));
    });
}
