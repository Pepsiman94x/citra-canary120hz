...
std::string GenerateVertexShader(const Pica::Shader::ShaderSetup& setup, const PicaVSConfig& config,
    ...
    if (config.state.use_geometry_shader) {
        ...
        out += GetVertexInterfaceDeclaration(true, config.state.gs_state.use_clip_planes,
                                             separable_shader);
        ...
        if (config.state.gs_state.use_clip_planes) {
...
static std::string GetGSCommonSource(const PicaGSConfigState& state, bool use_clip_planes,
                                     bool separable_shader) {
    std::string out = GetVertexInterfaceDeclaration(true, use_clip_planes, separable_shader);
    out += VSUniformBlockDef;
...
layout(triangle_strip, max_vertices = 3) out;
    ...
    out += GetGSCommonSource(config.state, config.state.use_clip_planes, separable_shader);
