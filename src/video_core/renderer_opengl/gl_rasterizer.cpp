...
RasterizerOpenGL::RasterizerOpenGL(Memory::MemorySystem& memory,
    ...
    : VideoCore::RasterizerAccelerated{memory}, driver{driver_},
      shader_manager{renderer.GetRenderWindow(), driver, true}, runtime{driver, renderer},
...
void RasterizerOpenGL::SyncTextureUnits(const Framebuffer* framebuffer) {
    ...
    const bool emulate_blend = state.EmulateColorBlend() || state.EmulateAlphaBlend();
    if (emulate_blend && !driver.HasShaderFramebufferFetch()) {
